...
#show: project.with(
  ...
  authors: (
    ...
    (name: "Anqur", contrib: "类型检查技术咨询", affiliation: "第七通用设计局"),
...
我发现 Algorithm W 真的很难理解，所以我写了这篇博客来帮助我理解它，希望这篇博客对你也有用。

本文面向使用过带有类型推导的语言（例如 Haskell, OCaml 和 Rust）并对类型推导感兴趣的人群。熟悉这些语言对于阅读本文来说不是必须的，但会有所帮助。

本文不涉及理论，也不会证明 Algorithm W。我更感兴趣的时 Algorithm W 实际上做了什么以及为什么这么做。

术语表和其他资源的连接在本文最下方（包括最初提出 Algorithm W 的论文）。

= 语言

我们用一门简单的程序设计语言来阐述 Algorithm W 的运作方式。这门语言非常简单，以至于它甚至不包含 `if` 语句或是数据结构这样的东西，但对于展示类型推导中所需的各种技巧而言已经足够。扩展语言使其具有更多特性，同时仍然应用这些技巧来推导类型也是完全可行的。
...
字面值的语法跟你想的一样，看起来就像 `55` 或者 `true`。变量的语法也一样，例如 `x` 或者 `foobar`。
...
函数抽象的语法就是字母 `λ` 加上单个参数（为什么只有一个参数？还是一样，稍后再说）后面跟着一个点，然后是一个作为函数体的表达式。例如：`λx. plus 1 x` 是一个将给定的数字加上 `1` 的函数。这个函数本身没有名字。将函数定义用括号括起来是很常见的。函数的函数体可以是另一个函数，例如 `λx. λy. plus x y`（实际上是 `(λx. (λy. (plus x y)))`）。
...
最后要指出一点：这门语言从语法上禁止了递归调用（在函数定义的内部函数名尚未被绑定，因此无法在函数定义内引用函数自身）。尽管如此，使用#link("https://en.wikipedia.org/wiki/Fixed-point_combinator")[不动点组合子]将非递归函数转换为递归函数还是可以的，尽管这个类型系统其实支持不了不动点组合子。
...
除了像这样的具体类型之外，还有类型变量。（它们通常写作单个字母，例如 $alpha$，$beta$ 等，有时也由字母和数字组成，例如 $alpha_0$，$beta_2$。）不要像编程语言里的变量那样看待它们，而是要像代数上的未知数那样看待它们：一个变量总归会有一个值（另一个类型），我们只是暂时不知道这个值是什么。例如，$alpha -> alpha$ 是一个函数类型，它接受某些东西（我们还没有指定），并返回同类型的某些东西。
...
就这么简单。这门语言中不包含任何类或者用户定义结构，所以没有表示这些东西类型的方式。

类型变量带来了一个使得类型推导更为有趣的小细节：泛型。让我们看看恒等函数（原封不动地返回其输入的函数）：`(λx. x)`。它的类型是 $alpha -> alpha$。我们在书写它的类型时必须使用类型变量 $alpha$，因为我们仅凭阅读函数定义尚不知道 `x` 的类型。也就是说，在实际调用这个函数的时候，`x` 会是一个具有某种类型的值。比如说我们用一个整数来调用这个函数：
...
这里，我们先是把 `square` 传给 `id` 函数（`id` 函数会原封不动地返回 `square`），然后又把 `123` 传给 `id`（也是原封不动地返回）。就程序的输出而言，这个程序等价于 #ml("let id = (λx. x) in square 123") （或者更简单地，`square 123`）。
...
还跟得上吧？好，现在是时候看看更高级的情况了。如果我们归一化 $alpha -> alpha$ 和 $"int" -> gamma$ 会怎么样？（如果我们将恒等函数应用于一个整数，我们就会看到这样的归一化，其中 $gamma$ 用来捕获函数的返回类型。）和之前一样，我们先归一化左边的 $alpha$ 和 $"int"$ 得到 ${ alpha : "int" }$。这里的情况有点微妙，我们得先把另一个 $alpha$（函数类型右边的哪个）换成 $"int"$，然后再继续操作：正如之前所说，当你取代一个类型变量的时候，你得把它所有出现的地方都换掉。所以在归一化左边之后，两个类型就变成了 $"int" -> "int"$ 和 $"int" -> gamma$。现在我们可以归一化右边的 $"int"$ 和 $gamma$ 了，这会得到 ${ gamma: "int" }$。最后我们结合两个替换，得到 ${ alpha: "int", gamma: "int" }$。恭喜，你刚刚检查出了对一个整数应用恒等函数会获得一个整数！
...
- 对具体类型（例如 $"int"$ 和 $"bool"$）应用替换会原封不动地返回该类型
- 对类型变量应用替换时
  - 如果这个类型变量已经有一个替换了，那么返回那个替换。比如说，如果类型变量是 $gamma$，而存在替换 ${ gamma: "bool" }$，则返回 $"bool"$；
...
#defn[要组合两个替换 $S_1$ 和 $S_2$，首先对 $S_2$ 中所有用于取代的类型应用 $S_1$，然后将 $S_1$ 中的所有条目加入 $S_2$ 中，除非 $S_2$ 中已经存在]。
...
记得看完算法之后回到这个例子，看看你能不能学以致用（这可能得花上个把小时。这个过程是有点味同嚼蜡，不过它能让你更好地理解这个算法的机制）。
...
2. 对类型环境中的所有类型应用替换 $S_1$ 以创建一个新的类型环境 $Gamma_1$，基于这个类型环境将类型 $tau_1$ 泛化成类型系列 $forall ... tau_1$。之后，我们不再使用 $Gamma_1$。
